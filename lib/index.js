...
function resolveSendMode(requestedMode, isResident) {
  ...
  if (mode === 'steer' || mode === 'inject') {
    if (!isResident && (requestedMode === undefined || requestedMode === null || requestedMode === '')) {
...
export function apply(ctx, config = {}) {
  ...
  tools.push(ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      await checkReasoningEffort(ctx, resolveEffectiveRoute(args, parent.options, undefined), args.reasoningEffort, exec.signal)
  ...
  tools.push(ctx.tools.register(defineTool({
    ...
    description:
      ...
      'When no mode is given and the child is not resident (cold), the delivery automatically falls back to followup so cold resume keeps working. ' +
    ...
    async execute(args, exec) {
      ...
      if (mode === 'followup') {
        ...
        return {
          ...
          note: fallback
            ? 'child is not resident; default steer fell back to followup (cold resume)'
            : cancelFirst
