...
import { detectCoreVersion, clientCapabilities } from './compat.js'
...
async function validateModelRoute(ctx, route, signal) {
  const provider = route && route.provider
  const model = route && route.model
  if (!provider || !model) {
    return { ok: true, skipped: true, reason: `provider/model route unavailable for validation (provider=${String(provider)}, model=${String(model)})` }
  }
  const llm = ctx && typeof ctx.get === 'function' ? ctx.get('llm') : undefined
  if (!llm || typeof llm.resolveModelInfo !== 'function') {
    return { ok: true, skipped: true, reason: 'llm route resolver unavailable' }
  }
  try {
    await llm.resolveModelInfo(String(provider), String(model), signal)
    return { ok: true }
  } catch (error) {
    const detail = error && error.message ? error.message : String(error)
    return { ok: false, reason: `provider "${String(provider)}" model "${String(model)}" is not routable: ${detail}` }
  }
}
...
/**
 * Compare an override with the latest request/header snapshot. This is a
 * detached status object for probe output, not a live session object.
 */
function compareOverrideToEffective(overrideSettings, effectiveConfig) {
  const override = overrideSettings || {}
  if (!effectiveConfig) return { status: 'pending', mismatches: [] }
  const effective = effectiveConfig
  const fields = ['provider', 'model', 'reasoningEffort', 'maxTokens', 'temperature']
  const mismatches = fields.filter((field) => (
    override[field] !== undefined && override[field] !== '' && effective[field] !== override[field]
  ))
  return { status: mismatches.length === 0 ? 'matched' : 'not_applied', mismatches }
}
...
export function apply(ctx, config = {}) {
  const coreVersion = detectCoreVersion()
  const caps = clientCapabilities(coreVersion)
  ctx.logger.info(`dsh-subagent-supervisor: core ${coreVersion || 'unknown'} (generation ${caps.generation}); host tools active; web panel ${caps.legacyClientRuntime && caps.apiProxyClient ? 'enabled' : 'inactive (alpha line lacks the legacy client runtime / ApiProxy)'}`)
  ...
  tools.push(ctx.tools.register(defineTool({
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          route_validation: { type: 'string', required: true },
        },
      },
      render: (args, value) => [{
        type: 'text',
    ...
    async execute(args, exec) {
      ...
      const route = { provider: agentOptions.provider, model: agentOptions.model }
      const routeVerdict = await validateModelRoute(ctx, route, exec.signal)
      if (!routeVerdict.ok) throw new Error(routeVerdict.reason)
      await checkReasoningEffort(ctx, route, args.reasoningEffort ?? applied.reasoningEffort, exec.signal)
      ...
      return {
        ...
        applied,
        route_validation: routeVerdict.skipped ? 'skipped' : 'validated',
      }
    },
  })))
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          override_status: { type: 'string', required: true },
          override_mismatches: { type: 'json', required: true },
          transcript: { type: 'string', required: true },
          pending_summary: { type: 'json' },
        },
      },
      render: (args, value) => {
        const overrideNote = value.override_status === 'not_applied'
          ? `\n⚠ override not applied: ${JSON.stringify(value.override_mismatches || [])}`
          : value.override_status === 'pending'
            ? '\n⏳ override pending: child has not emitted a request/header yet'
            : ''
        return [{
          type: 'text',
          text: `subagent ${value.subagent_id} [${value.status}]${value.effective_config ? ` config=${JSON.stringify(value.effective_config)}` : ''}${overrideNote}\n${value.transcript}`,
        }]
      },
    },
    async execute(args, exec) {
      ...
      const overrideSettings = overrides.get(childId)
      const overrideCheck = overrideSettings && Object.keys(overrideSettings).length > 0
        ? compareOverrideToEffective(overrideSettings, latestConfig)
        : { status: 'none', mismatches: [] }
      ...
      return definedOnly({
        ...
        effective_config: latestConfig
          ...
          : undefined,
        override_status: overrideCheck.status,
        override_mismatches: overrideCheck.mismatches,
    ...
    async execute(args, exec) {
      ...
      if (Object.keys(merged).length === 0) {
        return {
          subagent_id: childId,
          applied: {},
          route_validation: 'unchanged',
          note: 'no non-empty override fields supplied; nothing changed',
        }
      }
      // Fail fast: the merged route (args > existing overrides > parent) must
      // be registered before the override registry changes.
      const route = resolveEffectiveRoute(args, parent.options, merged)
      const routeVerdict = await validateModelRoute(ctx, route, exec.signal)
      if (!routeVerdict.ok) throw new Error(routeVerdict.reason)
      // The adapter may additionally reject the requested reasoning effort.
      await checkReasoningEffort(ctx, route, args.reasoningEffort, exec.signal)
      ...
      return {
        ...
        applied: merged,
        route_validation: routeVerdict.skipped ? 'skipped' : 'validated',
        note: routeVerdict.skipped
          ? 'applies from the child\'s next model request; route validation skipped because the llm resolver or complete route was unavailable; verify with subagent_probe'
          : 'applies from the child\'s next model request (agent/request waterfall); route validated and persisted; verify with subagent_probe',
      }
    },
  })))
