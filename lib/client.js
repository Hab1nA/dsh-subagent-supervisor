...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      // Auto-compat guard (0.1.1-rc.2 / 0.1.2-alpha.1 / 0.1.2-alpha.2): this
      // panel feeds on the legacy client runtime — the 'slots' face and the
      // ApiProxy client (ctx.connection.api / api.subagents.*). The alpha line
      // renamed dsh-client-runtime to dsh-client-modules and removed the
      // ApiProxy gateway, so those faces are absent there. When they are,
      // skip the panel instead of crashing the conversation view; host-side
      // tools need no client runtime and stay fully active.
      if (!ctx || !ctx.slots || typeof ctx.slots.inject !== 'function'
        || !ctx.connection || !ctx.connection.api) {
        console.warn('[dsh-subagent-supervisor] web panel skipped: this core line does not provide the legacy client faces (slots / connection.api ApiProxy). Host-side tools remain active.')
        return
      }
      try {
        ...
        console.warn('[dsh-subagent-supervisor] web panel failed to activate: ' + (error && error.message ? error.message : String(error)))
      }
    }
